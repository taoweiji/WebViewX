...
public class OpenLocationApi implements Api {
    ...
    public void invoke(ApiCaller caller) throws Exception {
        ...
        double longitude = caller.getParams().optDouble("longitude");
        double scale = caller.getParams().optDouble("scale");
        String name = caller.getParams().optString("name");
        String address = caller.getParams().optString("address");
        if ("gcj02".equals(type)) {
            JZLocationConverter.LatLng result = JZLocationConverter.gcj02ToWgs84(new JZLocationConverter.LatLng(latitude, longitude));
            latitude = result.latitude;
            longitude = result.longitude;
        }
        Uri uri = Uri.parse(String.format("geo:%s,%s?q=%s", latitude, longitude, address));
    ...
    static class JZLocationConverter {
